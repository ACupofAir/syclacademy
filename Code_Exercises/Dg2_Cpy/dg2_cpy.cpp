...
#define SG_SZ \
  (16) /* Arc770: Subgroup Sizes Supported: 8;16;32, while 8 threads per EU */
...
#define __LscLoadCachedVec(var, addr)                                \
  __asm__ __volatile__(                                              \
      "lsc_load.ugm.ca.ca   (M1, 16)  %0:d32x4  flat[%1]:a64"        \
      : "=rw"(reinterpret_cast<typename message_t::vector_t &>(var)) \
      : "rw"(addr)                                                   \
      : "memory")
#define __LscLoadL3CachedVec(var, addr)                              \
  __asm__ __volatile__(                                              \
      "lsc_load.ugm.uc.ca   (M1, 16)  %0:d32x4  flat[%1]:a64"        \
      : "=rw"(reinterpret_cast<typename message_t::vector_t &>(var)) \
      : "rw"(addr)                                                   \
      : "memory")
...
#define __LscStoreCachedVec(addr, var)                              \
  __asm__ __volatile__(                                             \
      "lsc_store.ugm.ca.ca  (M1, 16)  flat[%0]:a64  %1:d32x4"       \
      :                                                             \
      : "rw"(addr),                                                 \
        "rw"(reinterpret_cast<typename message_t::vector_t &>(var)) \
      : "memory")
...
static inline message_t _sum(message_t dst, message_t src) {
  ...
  return sycl::bit_cast<message_t>(sycl::bit_cast<math_t>(dst) +
                                   sycl::bit_cast<math_t>(src));
}

#if defined(__SYCL_DEVICE_ONLY__) && defined(__SPIR__)
static inline message_t sum(message_t dst, message_t src,
...
static inline void sync_data(char *src, message_t &data, int lid,
                             pattern_t pattern) {
...
// for a subgroup, it maintains a group of 16*4 uint32_t vec register, and they
// are viewed as 16*vec4-32bit registers (3, 7) means the 3rd 32bit element of
// the 7th vec4-32bit register (1, 15) means the 1st 32bit element of the 15th
// vec4-32bit register
//
...
// move the 6th 16bit element of 7th vec8-16bit to the 3rf 16bit element of 7th
// vec8-16bit seems not right...
...
static inline void send(char *dst, char *src, int lid, int req_workitems,
                        const ggml_type &dtype, pattern_t pattern,
                        size_t left_size) {
#if defined(__SYCL_DEVICE_ONLY__) && defined(__SPIR__)
  message_t data;
  int sz = sizeof(data);
...
// This function is used to copy data from src to dst using cpu memory as
// intermediate buffer
void dg2_bmg_ll256_cpy(void *dst, const void *src, size_t count,
  ...
  for (size_t chunk = 0; chunk < num_chunks; ++chunk) {
    ...
    q_src.submit([=](sycl::handler &h) {
      h.parallel_for(sycl::nd_range<1>(num_groups * l_sz, l_sz),
                     [=](sycl::nd_item<1> item)
                         [[sycl::reqd_sub_group_size(SG_SZ)]]
                     {
                       int lid = item.get_local_id(0);
                       send(static_cast<char *>(local_host_buf) + offset,
                            send_buf + offset, lid, num_workitems, dtype,
                            pattern, left_bytes);
                     });
    });
    ...
    q_dst.submit([=](sycl::handler &h) {
      h.parallel_for(sycl::nd_range<1>(num_groups * l_sz, l_sz),
                     ...
                     {
                       int lid = item.get_local_id(0);
                       recv(recv_buf + offset,
                            static_cast<char *>(local_host_buf) + offset, lid,
                            num_workitems, dtype, pattern, left_bytes);
                     });
    });
  }
}

void usm_memcpy(void *dst, const void *src, size_t size, sycl::queue &q_dst,
                sycl::queue &q_src) {
...
int main() {
  auto Devs = sycl::platform(sycl::gpu_selector_v)
  ...
  Queues[0].memcpy(dev0_ptr, &input[0],
                   N * sizeof(int32_t));  // dev1_ptr should be all zero
  ...
  // dg2_bmg_ll256_cpy(dev1_ptr, dev0_ptr, N, Queues[1], Queues[0],
  // GGML_TYPE_I32); Queues[0].wait(); Queues[1].wait();

  usm_memcpy(dev1_ptr, dev0_ptr, N * sizeof(int32_t), Queues[1], Queues[0]);
